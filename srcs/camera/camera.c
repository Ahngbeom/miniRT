...
void	init_camera(t_scene *scene, int fov)
{
	...
	scene->camera.u = vunit(vcross(scene->camera.vup, scene->camera.w)); // 카메라 좌표계 x축. vup과 w의 외적 : vup 벡터와 w 벡터의 수직 벡터
	scene->camera.v = vcross(scene->camera.w, scene->camera.u);	// 카메라 좌표계 y축. w와 u의 외적 : w 벡터와 u 벡터의 수직 벡터를 카메라의 y축으로 잡는다.
